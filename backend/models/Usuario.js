...
class Usuario {
    ...
    async create(email, password, nome, FKIDcargo, FKIDdepartamento) {
        try {
            var hashSenha = null;
            if (password != null)
                hashSenha = await bcrypt.hash(password, 8);//Criptografar senha

            return await knex.insert({ email, password: hashSenha, nome, FKIDcargo, FKIDdepartamento }).table(nomeTabela);//Cadastrar novo usuario
